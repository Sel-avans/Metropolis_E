<x-app-layout>
    ...
    <div class="flex flex-col xl:flex-row gap-4 w-full h-auto xl:h-full xl:overflow-hidden">
        ...
        <div id="library-column"
            class="library-sidebar relative w-full xl:w-auto p-4 xl:p-6 flex flex-col order-2 xl:order-1 xl:max-h-[73vh] xl:overflow-y-auto xl:overscroll-contain xl:flex-none xl:min-h-0">
            ...
            <section id="library-filters" class="shrink-0 mb-3">
                ...
                <input type="search" id="library-search" placeholder="Search by name…" autocomplete="off"
                        ...
                        <ul class="space-y-2 dark:text-white" role="list">
                            @foreach($items as $function)
                                <li class="library-item flex items-center gap-3 px-4 py-3 border border-gray-400 dark:border-gray-600 rounded cursor-pointer hover:border-blue-500 transition focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    ...
                                    tabindex="0" role="button" data-function-id="{{ $function->id }}"
                                    data-function-name="{{ $function->name }}" data-category="{{ $function->category }}"
                                    data-image="{{ asset($function->image) }}" aria-label="{{ $function->name }}">
                                    ...
                                    <button type="button"
                                        class="preview-info-btn p-2 text-slate-400 hover:text-sky-500 transition-colors z-10"
            ...
            <div id="library-preview-panel"
                ...
                role="dialog" aria-live="polite" aria-hidden="true" aria-labelledby="library-preview-title">
                <div id="library-preview-header"
                    class="library-preview-header shrink-0 flex items-start justify-between gap-3 px-4 pt-4 pb-2 border-b border-slate-600 bg-slate-900/95">
        ...
        {{-- MIDDLE: Buttons + Grid + Simulation Controls --}}
        <div
            class="flex flex-col w-full py-2 order-1 xl:order-2 overflow-x-hidden xl:flex-1 xl:min-h-0 xl:overflow-hidden">

            {{-- Buttons --}}
            <div class="grid grid-cols-2 xl:flex xl:flex-row gap-2 mb-2 shrink-0">
            ...
            {{-- City Grid --}}
            <div class="flex justify-center w-full shrink-0 pb-2">
                <div class="city-grid grid grid-flow-col grid-rows-3 gap-3" aria-label="City planning grid">
                    @for($col = 1; $col <= 4; $col++)
                        @for($row = 1; $row <= 3; $row++)
                            @php
                            ...
                            <div class="grid-cell relative flex border-2 items-center justify-center transition focus:outline-none focus:ring-2 focus:ring-blue-500 {{ $cellClasses }}"
                                ...
                                <div class="area-lock-explanation {{ $isApproved ? '' : 'hidden' }}" role="tooltip"
                                    aria-live="polite">
                                ...
                                @if(!empty($cell) && !empty($cell->function))
                                    ...
                                    @if(auth()->user() && (auth()->user()->role->name === 'City_planner' || auth()->user()->role->name === 'Administrator'))
                                        @if(!$isApproved)
                                            <button type="button"
                                                class="delete-btn absolute z-10 top-[2px] right-[2px] bg-red-600/80 text-white w-5 h-5 text-[14px] rounded cursor-pointer flex items-center justify-center">✖</button>
                                        @endif
                                    @endif
                                @endif
                            </div>
                    ...
                    <div class="flex justify-between items-center mt-2 text-xs text-gray-500 dark:text-gray-400">
                        ...
                        <div class="flex flex-col items-center gap-1">
                            ...
                            <div class="flex items-center gap-2 flex-wrap justify-center">
                                ...
                                <div
                                    class="flex items-center gap-2 bg-gray-100 dark:bg-gray-800 px-3 py-1 rounded-lg border border-gray-200 dark:border-gray-700 mt-2 sm:mt-0">
                                    <span
                                        class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Cycle</span>
                                ...
                                <p id="duration-validation-msg"
                                    class="hidden text-xs text-red-500 dark:text-red-400 w-full text-center mt-1">
