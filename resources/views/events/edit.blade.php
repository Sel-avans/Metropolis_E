                ...
                <form method="POST" action="{{ route('events.update', $event) }}">
                    ...
                    <div id="one_off_fields" class="mb-4 bg-gray-50 p-4 rounded border">
                        ...
                        <div class="mb-2">
                            ...
                            <input type="datetime-local" id="start_moment" name="start_moment" value="{{ old('start_moment', \App\Services\EventModifierService::toDatetimeLocalValue($event->start_moment) ?? '') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        </div>
                        
                        <div>
                            <label for="end_moment" class="block text-gray-700 font-bold mb-2">End Moment</label>
                            <input type="datetime-local" id="end_moment" name="end_moment" value="{{ old('end_moment', \App\Services\EventModifierService::toDatetimeLocalValue($event->end_moment) ?? '') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        </div>
                    </div>
