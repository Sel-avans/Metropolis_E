<x-app-layout>
    ...
    @if (session('success'))
        <div id="success-toast"
            class="fixed top-20 right-10 z-50 bg-green-500 text-white px-6 py-4 rounded-lg shadow-xl flex items-center gap-3 transition-opacity duration-500">
            ...
            <button onclick="document.getElementById('success-toast').style.display='none'"
                class="ml-4 focus:outline-none hover:text-green-200">
                                    ...
                                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                        ...
                                        @if($event->type === 'one-off')
                                            <div class="text-sm text-gray-900 whitespace-no-wrap">
                                                ...
                                                {{ $event->start_moment ? \App\Services\EventModifierService::formatForDisplay($event->start_moment) : 'N/A' }}
                                                <br>
                                                <span class="font-semibold">End:</span>
                                                {{ $event->end_moment ? \App\Services\EventModifierService::formatForDisplay($event->end_moment) : 'N/A' }}
                                            </div>
                                        @else
                                            <div class="text-sm text-gray-900 whitespace-no-wrap">
                                                <span class="font-semibold">Repeats:</span>
                                                {{ ucfirst($event->recurring_schedule) }}
                                                <br>
                                                <span class="text-gray-500">
                                                    From:
                                                    {{ $event->recurring_start_date ? \Carbon\Carbon::parse($event->recurring_start_date)->format('d-m-Y') : 'N/A' }}
                                        ...
                                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                            <div class="flex items-center space-x-4">
                                                <a href="{{ route('events.show', $event) }}"
                                                    class="text-blue-600 hover:text-blue-900 font-semibold">Details</a>
